...
class System:
    def __init__(self, primary_body, orbit_definitions, OrbitalEngine=KeplerianOrbit):
        ...
        self.orbits = [
            ...
            for [secondary_body, orbital_elements] in orbit_definitions
        ]
    ...
    def all_positions_relative_to(self, reference_body, t):
        if reference_body != self.primary_body:
            raise NotImplementedError(
                "Can only evaluate positions relative to primary body"
            )
        return {
            body: self._body_position_relative_to(body, reference_body, t)
    ...
    def all_trajectories(self, reference_body):
        if reference_body != self.primary_body:
            raise NotImplementedError(
                "Can only evaluate trajectories relative to primary body"
            )
        ...
        if subject_body == reference_body:
            ...
            orbit = next(
                (
                    ...
                    if orbit.secondary_body == subject_body
                    and orbit.primary_body == reference_body
                ),
                None,
            )
            if orbit is None:
                raise NotImplementedError(
                    "Can only evaluate the position of a secondary body relative to primary"
                )
